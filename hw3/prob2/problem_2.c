...
void lock(int tid){
	...
	for (i = 0; i < n_thread; ++i){
		if (i != tid){
			...
			while( (ticket[i] != 0) && ( ( ticket[tid] > ticket[i] ) || ( (ticket[tid] == ticket[i]) && (tid > i)) ) ){ /*yielding the CPU (instead of spinning)*/
				if (sched_yield() != 0){
					fprintf(stderr,"Error while yielding in thread %d.\n",tid);
					pthread_exit(NULL);
				}
			}
		}
...
int main(int agrc, char** agrv){
	...
	if (agrc != 3){
		printf("Usage: ./problem_2 <number of threads> <run time (seconds)>\n");
